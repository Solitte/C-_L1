...
// Заполните массив MxN и найдите сумму элементов на главной диагонали массива
...
int Diagonal2DArray(int[,] table)
{
    int sum = 0;
   //for (int i=0; i < table.GetLength(0) && i < table.GetLength(1); i++ )  sum = sum + table[i,i];

   for(int i = 0, j = 0; i < table.GetLength(0) && j < table.GetLength(1); i++, j++)
    {
        sum = sum + table[i,j];
    }
   return sum;
